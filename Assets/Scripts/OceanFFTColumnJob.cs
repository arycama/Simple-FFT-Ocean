...
public struct OceanFFTColumnJob : IJobParallelFor
{
    ...
    public OceanFFTColumnJob(int resolution, int passIndex, NativeArray<float4> butterflyLookupTable, NativeArray<float2> heightSource, NativeArray<float4> displacementSource, NativeArray<float2> heightResult, NativeArray<float4> displacementResult)
    {
    ...
    void IJobParallelFor.Execute(int index)
    {
        ...
        heightResult[index] = FFT(butterfly.zw, heightSource[x + (int)butterfly.x * resolution], heightSource[x + (int)butterfly.y * resolution]);
        displacementResult[index] = FFT(butterfly.zw, displacementSource[x + (int)butterfly.x * resolution], displacementSource[x + (int)butterfly.y * resolution]);
    }
}